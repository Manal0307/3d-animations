...
loader.load("./assets/jost.glb", function (gltf) {
  model = gltf.scene;
  ...
  //extra ; replace scene.add(model); par le block en dessousy§pà
  // Crée un pivot pour contrôler le positionnement global
  const pivot = new THREE.Group();
  pivot.add(model);
  scene.add(pivot);

  // Centre le modèle dans son propre espace
  const box = new THREE.Box3().setFromObject(model);
  const center = box.getCenter(new THREE.Vector3());
  model.position.sub(center); // center le mesh par rapport à lui-même

  // ➕ Maintenant tu peux déplacer le pivot
  pivot.position.y = -1.8; // Ajuste ici pour déplacer vers le bas
